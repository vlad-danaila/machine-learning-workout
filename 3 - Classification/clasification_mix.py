...
models['decision tree'] = sk.tree.DecisionTreeClassifier()
models['random forest'] = sk.ensemble.RandomForestClassifier()
models['naive'] = sk.naive_bayes.GaussianNB()
...
# Prepare decision boundry plot visualization
...
def plot_decision_boundary(y_grid, title, accuracy):
    ...
    plt.title('{} (accuracy {})'.format(title, accuracy))
    plt.show()

# Train, evaluate, visalize
for model_name, model in models.items():
    model.fit(x_train, y_train)
    y_pred = model.predict(x_test)
    conf_matrix = sk.metrics.confusion_matrix(y_test, y_pred)
    accuracy = np.sum(np.diag(conf_matrix)) / np.sum(conf_matrix)
    y_grid = model.predict(x_grid).reshape(x0_mesh.shape)
    plot_decision_boundary(y_grid, model_name, accuracy)